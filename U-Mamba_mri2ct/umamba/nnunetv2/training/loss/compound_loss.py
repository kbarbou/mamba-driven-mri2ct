...
class compound_loss(nn.Module):
    ...
    def forward(self, x, y, current_epoch=0):
        ...
        # Early detection of non-finite values; return inf so train_step can skip this batch
        if not torch.isfinite(x).all():
            print(f"[WARNING] Non-finite model output detected (max_abs={x.abs().max().item():.2f})")
            return torch.tensor(float('inf'), device=x.device, requires_grad=True)
