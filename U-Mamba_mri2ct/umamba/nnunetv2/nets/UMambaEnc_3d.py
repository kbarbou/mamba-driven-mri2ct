...
class UpsampleLayer(nn.Module):
    ...
    def forward(self, x, target_shape):
        ...
        x = F.interpolate(x, size=target_shape, mode=self.mode)
        x = self.conv(x)
        return x
...
class MambaLayer(nn.Module):
    ...
    def forward_channel_token(self, x):
        ...
        # channels as tokens → flatten spatial dims
        x_flat = x.flatten(2)          # [B, C, D*H*W]
        x_flat = x_flat.transpose(1, 2)  # [B, tokens, C]
...
class UNetResDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        seg_outputs = []
        for s in range(len(self.stages)):
            x_target_shape = skips[-(s+2)].shape[2:]
            x = self.upsample_layers[s](lres_input, x_target_shape)
