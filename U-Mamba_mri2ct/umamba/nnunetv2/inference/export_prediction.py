...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    # Regression task: only "background" label, no foreground classes.
    # Bypass apply_inference_nonlin (softmax/sigmoid) and argmax entirely —
    # save raw float32 logits (z-score CT values) directly to disk.
    if len(label_manager.foreground_labels) == 0:
        if isinstance(predicted_logits, torch.Tensor):
            predicted_logits = predicted_logits.float().cpu().numpy()
        else:
            predicted_logits = predicted_logits.astype(np.float32)
        # predicted_logits shape after resampling: (1, x, y, z) — take the single channel
        ct_volume = predicted_logits[0]
        del predicted_logits
        segmentation_reverted_cropping = np.zeros(properties_dict['shape_before_cropping'], dtype=np.float32)
        slicer = bounding_box_to_slice(properties_dict['bbox_used_for_cropping'])
        segmentation_reverted_cropping[slicer] = ct_volume
        del ct_volume
        segmentation_reverted_cropping = segmentation_reverted_cropping.transpose(plans_manager.transpose_backward)

        # Convert z-score predictions back to HU and clip to valid CT range.
        # Parameters match the ZScoreNormalization applied during nnUNet preprocessing.
        z_mean, z_std = -219.1395, 414.3167
        segmentation_reverted_cropping = segmentation_reverted_cropping * z_std + z_mean
        segmentation_reverted_cropping = np.clip(segmentation_reverted_cropping, -1024.0, 3071.0)

        torch.set_num_threads(old_threads)
        return segmentation_reverted_cropping  # float32 HU; write_seg must not cast to uint8
