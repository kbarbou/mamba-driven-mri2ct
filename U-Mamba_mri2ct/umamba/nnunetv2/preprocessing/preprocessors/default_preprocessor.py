...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        if has_seg:
            ...
            seg = self.modify_seg_fn(seg)
            assert seg.dtype == np.float32, "modify_seg_fn needs to return an array of type np.float32"
            print("[DEBUG] Seg file has been preprocessed")
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        if seg_file is not None:
            ...
            seg = None
        case_name = os.path.basename(image_files[0]).replace('.nii.gz', '')
        data_properties['case_name'] = case_name
    ...
    def modify_seg_fn(self, seg: np.ndarray, scaling="z_score") -> np.ndarray:
