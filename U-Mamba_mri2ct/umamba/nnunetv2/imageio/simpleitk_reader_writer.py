...
class SimpleITKIO(BaseReaderWriter):
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        ...
        # Preserve float dtype for regression tasks (e.g. z-score CT generation).
        # For segmentation tasks the input is always integer so the uint8 cast still applies.
        itk_image = sitk.GetImageFromArray(seg if np.issubdtype(seg.dtype, np.floating) else seg.astype(np.uint8))
