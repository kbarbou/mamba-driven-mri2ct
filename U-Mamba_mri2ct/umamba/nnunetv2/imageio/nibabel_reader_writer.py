...
class NibabelIO(BaseReaderWriter):
    ...
    def write_seg(self, seg: np.ndarray, output_fname: str, properties: dict) -> None:
        # revert transpose; preserve float dtype for regression tasks (e.g. z-score CT generation)
        seg = seg.transpose((2, 1, 0)) if np.issubdtype(seg.dtype, np.floating) else seg.transpose((2, 1, 0)).astype(np.uint8)
