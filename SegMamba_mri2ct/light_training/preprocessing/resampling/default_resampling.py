...
def compute_new_shape(old_shape: Union[Tuple[int, ...], List[int], np.ndarray],
                      ...
                      tol: float = 1e-5) -> tuple[np.ndarray, bool]:
    """
    Compute the new shape for resampling and verify if resampling is necessary.
    Returns (new_shape, do_resample) where do_resample is False if old/new spacing are effectively equal.
    """
    assert len(old_shape) == len(old_spacing) == len(new_spacing), "Shape and spacing must match dimensions"

    new_shape = np.array([int(round(o / os_ * ns)) for o, os_, ns in zip(old_shape, old_spacing, new_spacing)])

    spacing_equal = all(abs(os_ - ns) < tol for os_, ns in zip(old_spacing, new_spacing))
    shape_equal = all(ns_ == o for ns_, o in zip(new_shape, old_shape))

    do_resample = not (spacing_equal and shape_equal)

    if not do_resample:
        print(f"No resampling applied: old_spacing={old_spacing}, new_spacing={new_spacing}, shape unchanged={old_shape}")

    return new_shape, do_resample
...
def resample_data_or_seg_to_spacing(data: np.ndarray,
    ...
    new_shape, do_resample = compute_new_shape(shape, current_spacing, new_spacing)

    if do_resample:
        print(f"[INFO] Resampling data from current_shape={shape} current_spacing={current_spacing} to new_shape={new_shape} and new_spacing={new_spacing}")
        data_reshaped = resample_data_or_seg(data, new_shape, is_seg, order=order, order_z=order_z)
    else:
        print("[INFO] Skipping resampling; data already matches target spacing")
        data_reshaped = data
