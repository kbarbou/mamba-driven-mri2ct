...
def convert_predicted_image_to_original_shape(predicted_image: Union[torch.Tensor, np.ndarray], #arthur
    ...
    # Inverse Z-transform
    mean = properties_dict.get("target_mean", None)
    std = properties_dict.get("target_std", None)
    print("=== properties_dict ===")
    for k, v in properties_dict.items():
        print(f"{k}: {v}")

    if mean is not None and std is not None:
        predicted_resampled = predicted_resampled * std + mean
        print(f"[POST] Applied inverse z-transform (mean={mean}, std={std})")
        predicted_resampled = np.clip(predicted_resampled, -1024, 3000)
        predicted_resampled = predicted_resampled.astype(np.int16)
        print(f"[POST Clipping to [-1024,3000], Output data type: {predicted_resampled.dtype}]")
    else:
        print("[POST] No mean/std found, skipping inverse transform")
